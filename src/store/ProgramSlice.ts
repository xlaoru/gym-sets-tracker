...
export const getPrograms = createAsyncThunk("programs/getPrograms",
...
export const createProgram = createAsyncThunk("programs/createProgram", 
    async({ dayName, exercises, date }: Program, { rejectWithValue }) => {
        try {
            const response = await api.post("/api/programs", { dayName, exercises, date })
            return response.data
        } catch (error) {
...
export const editProgram = createAsyncThunk("programs/editProgram",
    async({ _id, dayName, exercises, date }: Program, { rejectWithValue }) => {
        try {            
            const response = await api.put(`/api/programs/${_id}`, { dayName, exercises, date })
            return response.data
        } catch (error) {
...
export const deleteProgram = createAsyncThunk("programs/deleteProgram",
...
const ProgramSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(deleteProgram.fulfilled, (state, action) => {
            state.status = "idle"
            state.programs = state.programs.filter((program) => program._id !== action.meta.arg._id)
        })
